...
The **least common multiple**, or $\\mathrm{lcm}$, of a list is the smallest natural number divisible by each entry of the list.  
Examples: $\\mathrm{lcm}(2,6,4) = 12$, $\\mathrm{lcm}(10,6,15,6) = 30$ and $\\mathrm{lcm}(11) = 11$.

Let $f(G, L, N)$ be the number of lists of size $N$ with $\\gcd \\ge G$ and $\\mathrm{lcm} \\le L$. For example:
